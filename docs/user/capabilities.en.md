...
`InfoOnlyDigital` states with documented `active`/`value` keys and observed 0/1
values can be recommended for on-change recording even without `is_analog`.
Existing native series are preferred, but their mapping to a particular state
and coverage of the requested period must be checked. Partial local recording

...
The main Admin UI shows enablement, the configured active-source count, the current SQLite database plus WAL file size, and links to **Local event history** in configuration and under **Help and developer tools**. The separate view shows local measurements first and loads visibility-dependent details afterward; an empty history needs no structure lookup. Control discovery starts only when you search for a source. Select an advertised state of a currently visible control; the source limit is 64 active pairs.

...
While the tab is visible, the page regularly checks a local change marker. If another browser adds or stops a source, or deletes its retained data, the page reloads the authorized source overview. When the marker is unchanged, no further Miniserver structure request is made.

...
| Klima/Lüftung | `IRoomControllerV2` | ja | ja | sichtbarer Timer-Modus `start_override`, `stop_override`; keine Kalender oder dauerhaften Temperaturen | `start_override` mit Eco-Temperatur und Bestätigungs-State real bestätigt; `stop_override` und weitere Modi bleiben nicht hardware-verifiziert |

...
| Klima/Lüftung | `IRoomControllerV2` | ja | ja | sichtbarer Timer-Modus `start_override`, `stop_override`; keine Kalender oder dauerhaften Temperaturen | `start_override` mit Eco-Temperatur und Bestätigungs-State real bestätigt; ein `stop_override` wurde angenommen, aber ohne frisches Bestätigungsereignis nicht bestätigt; weitere Modi bleiben nicht hardware-verifiziert |
| Klima/Lüftung | `Ventilation` | ja | ja | zeitlich begrenzter manueller Timer mit sichtbarem Modus, `stop_override`; keine Profile, Grenzen, Filterquittierung oder beliebigen Statusaktionen | `start_override` mit sichtbarem Modus und 60 Sekunden Laufzeit real bestätigt; `stop_override` bleibt nicht hardware-verifiziert |
